package org.tecnalia.proseco.app.io.xls.results;
...
public class ProjectResultExport {
	...
	public ProjectResultExport(Project project, EntityCache cache) {
		this.project = project;
		this.cache = cache;
		Collections.sort(project.getVariants(),
				new Comparator<ProjectVariant>() {
	...
	public void saveAnalysisResultToVCN(ProjectResultProvider result, String folderId) throws Exception {		
		Workbook workbook = new XSSFWorkbook();
		headerStyle = Excel.headerStyle(workbook);
		writeInfoSheet(workbook);
		Sheet inventorySheet = workbook.createSheet("LCI Results");
		...
		workbook.write(bos);
		bos.close();
		byte[] excel_content = bos.toByteArray();
		// Save the file into the VCN
		//CMISConnector.startSession();
		CMISConnector.saveExcelProjectInVCN(excel_content, folderId);
	}
	...
	private int writeVariantTable(Sheet sheet, int row) {
		...
		for (ProjectVariant variant : project.getVariants()) {
			...
			Excel.cell(sheet, row, 3, variant.getAllocationMethod().toString());
			Excel.cell(sheet, row, 4, variant.getProductSystem().getReferenceExchange().getFlow().getName());
			Excel.cell(sheet, row, 5, variant.getAmount());
			Excel.cell(sheet, row, 6, variant.getProductSystem().getReferenceExchange().getUnit().getName());
			row++;
		}
		return row;
	}
