...
public class AnalysisResultExport implements Runnable {
	...
	public AnalysisResultExport(ProductSystem system, FullResultProvider result) {
		this.system = system;
		this.result = result;
	}
	...
	public void saveAnalysisResultToVCN(String folderId) {
		try {
			prepareFlowInfos();
			prepareProcesses();
			prepareImpacts();
			...
			CMISConnector.saveExcelProductSystemInVCN(excel_content, folderId);
			success = true;
		} catch (Exception e) {
			log.error("Export failed", e);
			success = false;
